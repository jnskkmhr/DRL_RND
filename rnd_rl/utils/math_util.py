...
"""
Option 1:
direct policy shielding using CBF-OCP
action = CBF-QP(action_rl)
"""
...
"""
Option 2:
residual policy optimization using CBF-OCP
action = action_rl + action_residual
action_residual <- CBF-QP
"""
def get_cartpole_cbf_ocp_constraint(state, u_des)->tuple[torch.Tensor, torch.Tensor]:
    """
    define QP matrices for CBF-OCP for cartpole environment. 
    ...
    Here is OCP formulation:
    a* = a_rl + delta_a*
    delta_a* = argmin_a 0.5 * a^2
    s.t. Lf_h(x) + Lg_h(x) * (a_rl + a) + alpha * h(x) >= 0
    ...
    P = torch.ones(state.shape[0]).unsqueeze(-1).unsqueeze(-1)  # (N, 1, 1)
    q = torch.zeros_like(u_des)  # (N, 1)
    G = (-Lg_h_2).unsqueeze(-1).unsqueeze(-1)  # (N, 1, 1)
    h = (Lf_h_2 + Lg_h_2 * u_des.squeeze(-1) + alpha_2 * h_2).unsqueeze(-1)  # (N, 1)
